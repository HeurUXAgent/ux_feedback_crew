...
from ux_feedback_crew.tools.generate_feedback import _to_markdown
...
def _parse_task_output(raw: str) -> tuple[dict | None, str]:
    """
    The feedback tool now returns a JSON string directly.
    Try to parse it — if it fails fall back to the saved file.

    Returns:
        (feedback_json dict or None, feedback_markdown string)
    """
    raw = (raw or "").strip()

    # ── Try direct JSON parse (new behaviour) ────────────────────────────────
    try:
        data = json.loads(raw)
    ...
    # ── Fallback: raw is markdown, no structured data ─────────────────────────
    logger.warning("[PARSE] Could not extract structured JSON from task output — returning raw as markdown only")
    return None, raw


def _load_feedback_json(evaluation_id: str) -> dict | None:
    """
    Last-resort fallback: read the JSON file written by the tool.
    Only used if _parse_task_output fails for both paths.
    """
...
async def analyze_and_wireframe_s3(
    ...
    try:
        ...
        # Last resort — try the saved file
        if feedback_json is None:
            logger.warning("[PIPELINE] Falling back to saved JSON file")
            feedback_json = _load_feedback_json(job_id)

        logger.info(f"[PIPELINE] feedback_json keys: {list(feedback_json.keys()) if feedback_json else 'None'}")
        logger.info(f"[PIPELINE] feedback_items count: {len(feedback_json.get('feedback_items', [])) if feedback_json else 0}")

        return {
            "evaluation_id":  job_id,
            "image_url":      image_url,
            "feedback":       feedback_md,        # markdown for display fallback
            "feedback_json":  feedback_json,      # structured JSON for Flutter cards
...
async def regenerate_wireframe(
    ...
    original_feedback    = (
        ai.get("feedback_report", {}).get("markdown", "")
...
async def submit_hitl_feedback(
    ...
    if body.user_action not in ("agree", "disagree", "modify"):
        raise HTTPException(
            status_code=400,
            detail="user_action must be: agree | disagree | modify",
        )
    save_hitl_response(
